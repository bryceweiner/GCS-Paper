...
GAMMA = Decimal('1.89e-29')        # Universal branching rate (s⁻¹)
...
def h0_qbu_late(z):
    """QBU prediction for late universe"""
    z_dec = Decimal(str(z))
    t = cosmic_time(z_dec)
    ...
    # Calculate H0 with quantum branching enhancement
    h0_value = H0_PLANCK * (Decimal('1') + enhancement)
    
    return float(h0_value)

def calculate_qbu_uncertainty_late(z):
    """QBU uncertainty in late universe
    
    Uncertainty increases at late times due to:
    1. Base measurement uncertainty
    2. Quantum branching uncertainty that grows with time
...
for z in test_z:
    ...
    qbu_unc = calculate_qbu_uncertainty_late(z)
    print(f"\nAt z = {z}:")
    print(f"ΛCDM uncertainty: {lcdm_unc:.6f}")
    print(f"QBU uncertainty:  {qbu_unc:.6f}")
    print(f"Difference:       {abs(qbu_unc - lcdm_unc):.6f}")
...
qbu_h0 = np.array([h0_qbu_late(z) for z in z_values])
qbu_unc = np.array([calculate_qbu_uncertainty_late(z) for z in z_values])
...
# Plot QBU with uncertainty
plt.plot(z_values, qbu_h0, 'r-', label='QBU prediction', zorder=2, linewidth=2)
plt.fill_between(z_values, 
                qbu_h0 - qbu_unc,
                qbu_h0 + qbu_unc,
                color='red', alpha=0.2, label='QBU 1σ uncertainty')
...
for z in test_points:
    ...
    print(f"QBU:  {qbu_h0[idx]:.3f} ± {qbu_unc[idx]:.3f}")
    print(f"Band widths:")
    print(f"ΛCDM: {(2 * lcdm_unc[idx]):.3f}")
    print(f"QBU:  {(2 * qbu_unc[idx]):.3f}")
...
def calculate_chi2(model_type, data_point):
    ...
    if model_type == 'lcdm':
        ...
        model_value = Decimal(str(h0_qbu_late(float(z_dec))))
        model_uncertainty = Decimal(str(calculate_qbu_uncertainty_late(float(z_dec))))
...
chi2_qbu = sum(calculate_chi2('qbu', point) for point in late_data)
...
for z in test_z:
    ...
    print(f"QBU plot value:  {qbu_h0[idx]:.6f}")
    print(f"Difference:      {abs(qbu_h0[idx] - lcdm_h0[idx]):.6f}") 
