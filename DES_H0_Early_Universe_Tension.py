...
# QBU Model - completely independent
def h0_qbu_early(z):
    """QBU prediction for early universe with proper precision"""
    z_dec = Decimal(str(z))
    t = cosmic_time(z_dec)
...
# QBU predictions (with proper deviation)
qbu_h0 = np.array([h0_qbu_early(z) for z in z_values])
qbu_unc = np.array([calculate_qbu_uncertainty(z) for z in z_values])
...
for z in [100, 500, 1089.8, 2000]:
    ...
    print(f"QBU H0:  {h0_qbu_early(z):.6f}")
    print(f"Difference: {abs(h0_lcdm_early(z) - h0_qbu_early(z)):.6f}")
...
def calculate_chi2(model_type, data_point):
    ...
    if model_type == 'lcdm':
        ...
        t = cosmic_time(z_dec)
        branching_term = GAMMA * t * (Decimal('1') + z_dec).sqrt() * Decimal('1e-3')
        model_value = H0_PLANCK * (Decimal('1') - branching_term)
        
        base_unc = Decimal('0.54')
        quantum_unc = Decimal('0.1') * branching_term
